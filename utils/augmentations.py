# Inherited from PointNet++ source code
...
def rotate_point_cloud(batch_data, rotation_axis="y"):
    ...
    for k in range(batch_data.shape[0]):
        ...
        rotmax_ = [[cosval, 0, sinval],
                  [-sinval, 0, cosval]]

        assert rotation_axis in ['x', 'y', 'z']
        target_axis = ord(rotation_axis)-ord('x')
        axes = np.arange(3)
        axes = np.delete(axes, [target_axis])
        rotation_matrix = np.eye(3)
        rotation_matrix[axes] = rotmax_
...
def test():
    ...
    pcd_colored_org = vis.pc_utils.points2PointCloud(vis.paint_colormap(pc, labels))
    ...
    for name, aug, args in target_augmentations:
        ...
        pcd_colored = vis.pc_utils.points2PointCloud(vis.paint_colormap(pc, labels))
