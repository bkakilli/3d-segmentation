...
def create_hierarchy(x, p, num_groups, group_size):
    ...
    samples = fp_sampling(p, num_groups)    # (batch_size, num_groups) (In every batch, we choose num_goups point to cover all points)
    idx = knn(p, k=group_size, selection=samples)   # (batch_size, num_group(此处的num_group为cover当前点云的那些店,也就是num_of_group), group_size)
    ...
    #hierarchy's size is (batch_size, num_groups, group_size, embeddings). This is based on the feature after 2 convolution layer
    #new_p is every batch's centroid's cartisan coordinate,(batch_size,num_groups,3)
    return hierarchy, new_p
